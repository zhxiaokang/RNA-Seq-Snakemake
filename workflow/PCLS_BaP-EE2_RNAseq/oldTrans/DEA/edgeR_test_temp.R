...
countTable <- read.table('./Mix_high_VS_BaP_high', header = T)
...
toptag <- topTags(lrt, n = length(geneList), adjust.method="BH", p.value = 1)
deg <- toptag$table

# save the DEGs to file
write.csv(deg, 'dea_Mix_high_VS_BaP_high')
